...
`sbml2cellml` follows the convention for libraries: it only gets loggers and logs
...
All loggers are therefore below the `sbml2cellml` logger, so an application
configures them in one place:

```python
import logging

logging.getLogger("sbml2cellml").setLevel(logging.WARNING)
```
...
from sbml2cellml.console import console as default_console

#: name of the logger all loggers of the package are below
PACKAGE_LOGGER = "sbml2cellml"
